...
## ggplot(data=df) +
##   aes(x=X, y=Y) +
##   geom_segment(aes(x=START_POINT_X, xend=END_POINT_X,
##                    y=START_POINT_Y, yend=END_POINT_Y)) +
##   geom_point( color="orange", size=7) +
##   coord_flip()
...
p11a <- ggplot(data = d) +
  geom_mosaic(aes(x = product(Tree,City),             ## NEED ALL COMBOS PRESENT
                  fill=Tree),
              divider=mosaic("v")) +
  labs(title='mosaic v')
p11a
